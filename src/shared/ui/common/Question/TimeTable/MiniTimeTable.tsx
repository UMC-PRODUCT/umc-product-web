        ...
        <S.MainAreaWrapper>
          <S.MainArea ref={mainAreaRef}>
            <S.HeaderRow $cols={dates.length}>
              {dates.map((d) => {
                return (
                  <S.HeaderCell key={d}>{`${formatDateLabel(d)} ${getDayLabel(d)}`}</S.HeaderCell>
                )
              })}
            </S.HeaderRow>
...
export const MiniTimeTable = forwardRef<HTMLDivElement, ScheduleSelectorProps>(TimeTableComponent)
MiniTimeTable.displayName = 'MiniTimeTable'
